...
const CreatePassword = () => {
  ...
  const { isLoading, data: key } = useQueryData(
    `${apiVersion}/controllers/developers/settings/system-users/key.php?key=${paramKey}`, // endpoint
    "get", // method
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        `${apiVersion}/controllers/developers/settings/system-users/set-password.php`,
        "post",
        values,
      ),
    onSuccess: (data) => {
        ...
        >
          <div className="w-96 p-6">
            <div className="flex justify-center items-center flex-col">
              <img src={ftcLogo} alt="FTCD" className="w-48 h-auto" />
            </div>
